...
void loop() 
{
 ...
 // Serial.println("request sent");
  ...
  pixels.clear();
  if( (intPM10 > 0) && (intPM10 <= fineDustClean))
  {
    pixels.setPixelColor(0, pixels.Color(0, 0, 150)); // BLUE
    pixels.show(); 
  }
  else if( (intPM10 > fineDustClean) && (intPM10 <= fineDustGood))
  {
    pixels.setPixelColor(0, pixels.Color(0, 150, 0)); // GREEN
    pixels.show(); 
  }
  else if(intPM10 > fineDustGood)
  {
  ...
  {
    String valMode = firebaseData.stringData();
    if(valMode == "1")
    {
      if( (intPM10 > 0) && (intPM10 <= fineDustClean)) {digitalWrite(fan, LOW);}
      else if( (intPM10 > fineDustClean) && (intPM10 <= fineDustGood)) { digitalWrite(fan, HIGH);}
      else if(intPM10 > fineDustGood) { digitalWrite(fan, HIGH); }
    }
    else if (valMode == "0")
    {
      if(Firebase.getString(firebaseData, "FAN"))
      {
        String valFan = firebaseData.stringData();
        if(valFan == "1") digitalWrite(fan,HIGH);
        else if (valFan == "0") digitalWrite(fan,LOW);
        //else digitalWrite(fan,LOW);b
      }
    }
  }
 delay(1000);
